...
function adminPanel() {
  ...
  useEffect(async () =>  {
      const categories = await axios.get('https://hilal-taraz.kz/api/category').then(res => res.data);
      setCards(categories);
      setLoading(false);
  }, []);
      ...
      <Layout style={{ height: '100%', minWidth:'100vh', minHeight:'100vh' }}>
        ...
        <Layout className="site-layout" style={{minWidth:'100vh', minHeight:'100vh'}}>
          ...
          <Content
            ...
            {React.createElement(select == "prods" ? Prods : Category, {categories: cards, loading: loading }) }     
          </Content>
        </Layout>
      </Layout></div>)
}
