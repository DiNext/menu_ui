...
export default function AdminPanelProds({categories, loading}) {
    ...
    async function onSelect(e) {
      ...
      const res = await axios.get(`https://pinta-taraz.kz/api/prods/find?id=${id}`, config);
      setData(res.data);
    }
     ...
     async function onDelete(record){
      ...
      await axios.delete(`https://pinta-taraz.kz/api/prods?id=${id}`, config);

      const idCat = selectedNode;
      const res = await axios.get(`https://pinta-taraz.kz/api/prods/find?id=${idCat}`, config);
      setData(res.data);
